...
class OrderListService extends Base {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                list.push({
                    ...
                    order_placed_on: element?.order_date === "" ? "" : moment(element.order_date, "DD-MM-YYYY").format("DD MMM YYYY"),
                    ...
                    total_piece: element?.total_pcs,
                    delivery_date: element?.delivery_date === "" ? "" : moment(element?.delivery_date, "DD-MM-YYYY").format("DD MMM YYYY"),
    ...
    populateRequestData(data) {
        // console.log("populateRequestData ===> ",data);
        return new Promise((resolve, reject) => {
            let list = []
            for (let index = 0; index < data.length; index++) {
                const element = data[index];
    ...
    _getRequestService(param) {
        ...
        return new Promise((resolve, reject) => {
            this.post(REQUEST_LIST_SLUG, param).then(async response => {
                ...
                if (response.data.data.success == false) {
                    store.dispatch(storeOrderListAction([]))
                    reject({ message: "No Request Available" })
                    return
                }
                const data = response?.data?.data?.data
